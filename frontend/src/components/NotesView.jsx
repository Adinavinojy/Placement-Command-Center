...
export default function NotesView({ onUpdate }) {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const res = await fetchAuth(`${API_BASE}/api/documents`);
  ...
  const fetchIndexStatus = useCallback(async () => {
    try {
      const res = await fetchAuth(`${API_BASE}/api/notes/status`);
      const data = await res.json();
      setIndexStatus(data);
      return data;
    } catch (e) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await fetchAuth(`${API_BASE}/api/vault/upload`, {
        method: 'POST',
        body: formData
      });
  ...
  const handleDelete = async (filename) => {
    ...
    try {
      await fetchAuth(`${API_BASE}/api/documents/Notes/${filename}`, {
        method: 'DELETE'
      });
      await fetchDocuments();
  ...
  const handleSearch = async () => {
    ...
    try {
      setIsSearching(true);
      const res = await fetchAuth(`${API_BASE}/api/notes/search`, {
        method: 'POST',
  ...
  const handleReindex = async () => {
    try {
      setReindexing(true);
      await fetchAuth(`${API_BASE}/api/notes/reindex`, { method: 'POST' });
  ...
  const getPreviewUrl = (filename) => {
    ...
    return `${API_BASE}/api/documents/Notes/${filename}?token=${token}`;
  };
